...
?>
        ...
        <!-- <form action='index.php' method='post' class="flex w-full justify-start">
            <button class="text-white text-lg bg-red-600 focus:outline-none focus:ring-4 focus:ring-green-300 font-bold rounded-full py-2 text-center mb-2 w-48" type='submit' name='logout'>Déconnexion</button>
        </form> -->


        <form method='post' action='index.php' class="flex justify-center w-full px-4">
            <table class="">
<?php              
                $case = 1;
...
?>
                            <td class="border border-sky-600" style="height: 100px; width: 100px;">
                                <button style ="height: 100%; width: 100%;" <?php disabled($case) ?> type="submit" name="<?php echo $case ?>" value=""  style="width: 50px; height: 50px;">
                                    <div class="flex w-full h-full justify-center items-center text-5xl <?php Color($case) ?>"><?php affichage($case) ?></div>
                                </button>
                            </td>
<?php  
                            $case++;
